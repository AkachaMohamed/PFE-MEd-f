...
const TrainerScreen=(props)=> {
  ...
  const datum =  DataImport("trainers").then((ee)=>{
      setData(ee);
      setRefreshing(false);
      setRefreshing(false)
    })
  ...
  // Suppression d'un entraineur
   const handleDelete = (data) => {    
    const x=DeleteData("trainers",data).then((reponse)=>{ 
      Alert.alert(
        "",
  ...
  useEffect( () => {
    function fetchData() {
        const contenu=  DataImport("trainers");
        return contenu
    }
    try {
     setLoading(true);
  ...
  return (
    <Screen>
      ...
      <TouchableOpacity
          ...
          activeOpacity={1.0}
          onPress={()=>props.navigation.navigate("Updat_Ent")}
          ...
          <ListItem
            ...
            renderRightActions={() => (   
              <View> 
               ...
               <ListItemSeparator/>
               <ListItemUpdatAction 
                 onPress={() =>props.navigation.navigate("Updat_Ent",{item:item})}
               />
               </View>
            )}
          />
