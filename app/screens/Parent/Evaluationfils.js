...
const TrainerScreen=(props)=> {
  ...
  //Importation de données apres refresh 
   /*console.log('****************************************')
   const y=selone("evaluation","Akacha@outlook.com").then((ee)=>{
    setData(ee);
   setRefreshing(false);
   setRefreshing(false) }
)*/
  ...
  useEffect( () => {
    function fetchData() {
        const contenu=  selone("evaluation");
        return contenu
    }
    try {
     setLoading(true);
      const y =  fetchData().then((ee)=>{
        setData(ee);        
        setRefreshing(false)
      })      
    } catch (error) {      
 ...
 if (!data1 || data1.length==0)
 {
  return (
    <Screen>
      <View><Text>Aucun Evaluation pour le moment  dans la base de données</Text></View>
