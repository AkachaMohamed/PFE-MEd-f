...
const AdminScreen=(props)=> {
  ...
  const datum =  DataImport("admins").then((ee)=>{
      setData(ee);
      setRefreshing(false);
      setRefreshing(false)
    })
  ...
  // Suppression d'un entraineur
   const handleDelete = (data) => {    
    const x=DeleteData("admins",data).then((reponse)=>{ 
      Alert.alert(
        "",
  ...
  useEffect( () => {
    function fetchData() {
        const contenu=  DataImport("admins");
        return contenu
    }
    try {
     setLoading(true);
 ...
 if (!data1 || data1.length==0)
 {
  return (
    <Screen>
      <View><Text>Aucun Admin dans la base de données</Text></View>
      ...
      <TouchableOpacity
          ...
          activeOpacity={1.0}
          onPress={()=>props.navigation.navigate("Update_ad")}
          ...
          <ListItem
            ...
            renderRightActions={() => (   
              <View> 
               ...
               <ListItemSeparator/>
               <ListItemUpdatAction 
                 onPress={() =>props.navigation.navigate("Update_ad",{item:item})}
               />
               </View>
            )}
          />
